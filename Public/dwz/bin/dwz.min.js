	...
	ajaxError:function(xhr, ajaxOptions, thrownError){
		if (alertMsg) {
			/* alertMsg.error("<div>Http status: " + xhr.status + " " + xhr.statusText + "</div>" 
				...
				alertMsg.error("<div>网络不给力，请待会儿再试试~</div>");
		} else {
...
;(function($) {
  ...
  var TinySelect = {
	...
	createSelect: function($el) {
		...
		this.state.container = $("<div id='"+this.config.boxId+"' style='background: rgba(0, 0, 0, 0) url(\"../Public/dwz/themes/default/images/search-bg.gif\") no-repeat scroll 0 0;'></div>").
			addClass("tinyselect").
			css({ "width": "135px"});
		...
		$('body').append(this.state.dropdown);
...
(function($){
    $.fn.mailAutoComplete = function(options){
        ...
        if(settings.autoClass && $("#mailListAppendCss").size() === 0){
            $('<style id="mailListAppendCss" type="text/css">.mailListBox{border:1px solid #369; background:#fff; font:12px/20px Arial;}.mailListDefault{padding:5px;cursor:pointer;white-space:nowrap;}.mailListFocus{padding:5px;cursor:pointer;white-space:nowrap;background:#369;color:white;}.mailListHlignt{color:red;}.mailListFocus .mailListHlignt{color:#fff;}</style>').appendTo($("head"));    
        }
...
(function($){
	if ($.validator) {
		...
		$.validator.addMethod("hexadecimal", function(value, element) {
			return this.optional(element) || /^[A-F 0-9]*$/.test(value);
		}, "请输入正确的16进制格式");

		$.validator.addMethod("doubles", function(value, element) {
			return this.optional(element) || /^[0-9]*[.]{0,1}[0-9]{0,2}$/.test(value);
		}, "请输入大于0的数字,保留两位小数");
				...
				if (tree.size()>0) {
					...
					$(">div>div:first,>div>a", node).click(function(){
						...
						if (!isA || tree.is(":hidden")) {
							var pare = $this.closest('.tree');//20151130	gzy
							pare.find('ul').slideUp();	
							pare.find('.first_collapsable').removeClass().addClass("first_expandable");
							pare.find('.collapsable').removeClass().addClass("expandable");
							pare.find('.last_collapsable').removeClass().addClass("last_expandable");
...
function initLayout(){
	...
	$("#container .tabsPageContent").height(iContentH - 7).find("[layoutH]").layoutH();	//gzy 控制右边大框
	$("#sidebar, #sidebar_s .collapse, #splitBar, #splitBarProxy").height(iContentH + 25);
...
function initUI(_box){
	...
	$("a[target=navTab]", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var title = $this.attr("res_title") || $this.attr("title") || $this.text();
	...
	$("a[target=selectOne]", $p).each(function(){
	  ...
	  $(this).click(function(event){
	   ...
	   if (!ids) {
		...
		if (title) {
			alertMsg.confirm(title, {okCall: title});
		} else {
			var $this = $(this);
			...
			//20160223 gzy
			var check = eval($this.attr("check") || "false");
			if(check == true){
				var code  = 0;
				var mesg  = '';
				$.ajax({type:"POST",url:url+'?ids='+ids,async:false,
					success: function(_data){
						code = _data.statusCode;
						mesg = _data.message;
					}
				});
				if(code == 300){
	...
	$("a[target=dialog]", $p).each(function(){
		$(this).click(function(event){
			...
			//20160223 gzy
			var port = eval($this.attr("export") || "false");
			if(port == true){
				url = url + $('div.unitBox:visible').find('#exportdata').val();
			}
			$.pdialog.open(url, rel, title, options);
	...
	$("div.pagination", $p).each(function(){
		var $this = $(this);
		$this.pagination({
			checkPage:$this.attr("checkPage") ? '1' : '0',
...
(function($){
	$.fn.extend({
		theme: function(options){
			...
			var _themeHref = "dwz/"+ op.themeBase + "/#theme#/style.css";
			return this.each(function(){
...
(function($){
	$.pdialog = {
		_op:{height:300, width:580, minH:40, minW:50, total:20, max:false, mask:false, resizable:false, drawable:true, maxable:false,minable:true,fresh:true},
		_current:null,
		_zIndex:42,
		getCurrent:function(){
		...
		open:function(url, dlgid, title, options) {
			...
			var urlparam = options.urldata || {};		//26日扩展(用于向弹框传递数据)
			var tdialog  = options.tdialog || false;	//双弹框处理 查找带回
			
			//重复打开一个层
			if(dialog && tdialog==false) {
				if(dialog.is(":hidden")) {
					dialog.show();
				}
				if(op.fresh || url != $(dialog).data("url")){
					...
					var jDContent = dialog.find(".dialogContent");
					jDContent.loadUrl(url, urlparam, function(){
						jDContent.find("[layoutH]").layoutH(jDContent);
						$(".pageContent", dialog).width($(dialog).width());
		...
		return this.each(function(){
			...
			$trs.hoverClass().each(function(){
				...
				$tr.click(function(){
					$trs.filter(".selected").removeClass("selected");
					$tr.addClass("selected");
					//$tr.find('.checkboxval').attr('checked', true);		//gzy
					var sTarget = $tr.attr("target");
					if (sTarget) {
						if ($("#"+sTarget, $grid).size() == 0) {
							$grid.prepend('<input id="'+sTarget+'" type="hidden" />');
		...
		lookup: function(){
			return this.each(function(){
				var $this = $(this), options = {mask:true, 
					width:$this.attr('width')||640, height:$this.attr('height')||380,
					maxable:eval($this.attr("maxable") || "false"),
					resizable:eval($this.attr("resizable") || "false"),
					tdialog:eval($this.attr("tdialog") || "false")
				};
				$this.click(function(event){
		...
		select: function(_name, _type, _parent){
			...
			switch(_type){
				case "invert":
					...
					break;
				case "none":
					//console.log($parent.find('tr.selected').length);return; gzy
			...
			return this.each(function(){
				...
				$(op.selector, box).click(function(){
					...
					if (options.is(":hidden")) {
						...
						var top = box.offset().top+box[0].offsetHeight;	//gzy
