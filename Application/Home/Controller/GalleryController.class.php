...
class GalleryController extends HomeController {
	...
	**/
	public function host() {
		...
		//支付渠道 0:上福 1:浙星 2:JAVA 3:民生 4:华宝 5:威富通 6:易联众 7:通莞聚合 8:杉德 9:ISV
		$this->assign('channel_list',			C('CHANNEL_LIST'));
	...
	**/
	public function host_show() {
		...
		$field = 'ho.*,ha.DAY_MAXAMT,ha.SIGN_MAXAMT,ha.HOST_PPP_FLAG,ha.AUTH_TRANS_MAP,hc.HOST_SETTLE_FLAG,hc.SETTLE_OFF_AMT,hc.SETTLE_FREE_AMT,hc.SETTLE_TOP_AMT,hc.SETTLE_T,hc.SETTLE_T_UNIT,hc.SETTLE_FLAG,hc.SETTLE_OFF_FEE,hc.SETTLE_FEE';
	...
	public function host_edit() {
		...
		if($post['submit'] == "host_edit") {
			...
			$hostdata = array(
				...
				'END_TIME'			=>	date('Ymd', strtotime($post['END_TIME'])).'235959',
				'KEY_TYPE'			=>	$post['KEY_TYPE'],
				'HOST_TRANS_TIME'	=>	$post['HOST_TRANS_TIME'],
				'HOST_DF_TIME'		=>	$post['HOST_DF_TIME'],							 
			);
	...
	public function hauth_edit() {
		...
		if($post['submit'] == "hauth_edit") {
			...
			if($post['flag'] == 0){
				...
				$hauth_copy['DAY_MAXAMT'] = $post['DAY_MAXAMT'] ? setMoney($post['DAY_MAXAMT'], '6') : 100000000;
				$hauth_copy['SIGN_MAXAMT'] = $post['SIGN_MAXAMT'] ? setMoney($post['SIGN_MAXAMT'], '6') : 100000000;
				...
				$hauth_copy = array(
					...
					'DAY_MAXAMT'	=>	$post['DAY_MAXAMT'] ? setMoney($post['DAY_MAXAMT'], '6') : 100000000,
					'SIGN_MAXAMT'	=>	$post['SIGN_MAXAMT'] ? setMoney($post['SIGN_MAXAMT'], '6') : 100000000,
				);
		...
		$info['DAY_MAXAMT']  = $hauth_data['DAY_MAXAMT'];
		$info['SIGN_MAXAMT']  = $hauth_data['SIGN_MAXAMT'];
		//交易开通
		$trans_list = D('MTrans')->getTranslist('TRANS_FLAG1 = 1 and TRANS_STATUS = 0','TRANS_MAP_ID,TRANS_NAME');	
		$this->assign('trans_list',			$trans_list);				//交易开通
	...
	**/
	public function hshop_edit() {
		...
		if($post['submit'] == "hshop_edit") {
			...
			//不需要更新进件表信息
			// if($finddata['HSHOP_NO']!=$resdata['HSHOP_NO'] && $resdata['HOST_MAP_ID']>0){
				// $pieceModel = M("shop_intopieces");
				// $pieceModel->where(array('HOST_MAP_ID'=>$resdata['HOST_MAP_ID'], 'SHOP_ID1'=>$finddata['HSHOP_NO']))->save(array('SHOP_ID1'=>$resdata['HSHOP_NO'], 'KEYT'=>$resdata['KEYT']));
			// }
			$this->right($res['msg'], 'closeCurrent', \Cookie::get ( '_currentUrl_' ), $_REQUEST['navTabId']);
		}
	...
	public function hpos() {
		...
		if($post['submit'] == "hpos"){
			...
			if($home['HOST_MAP_ID']>0){
				$where .= " and ho.HOST_MAP_ID = '".$home['HOST_MAP_ID']."'";
			}
			//分页
		...
		//通道列表
		$host = D($this->MHost)->getHostlist($where, 'HOST_MAP_ID,HOST_NAME');
		foreach($host as $val){
			$host_list[$val['HOST_MAP_ID']] = $val['HOST_NAME'];
		}
		$this->assign('host_list',			$host_list);
