	...
	**/
	public function statiscard() {
		...
		if($post['submit'] == "statiscard"){
			...
			$fiRow    = ($_REQUEST[C('VAR_PAGE')] ? $_REQUEST[C('VAR_PAGE')]-1 : 0) * (C('PAGE_COUNT') - 1);
			$liRow    = ($_REQUEST[C('VAR_PAGE')] ? $_REQUEST[C('VAR_PAGE')] : 1)   * (C('PAGE_COUNT') - 1);
			$list_sql = "select sql_no_cache count(1) as TOTAL,sum(if(CARD_STATUS='1',1,0)) as KUCUN,BRANCH_MAP_ID,PARTNER_MAP_ID from k_vipcard 
						where 
						PARTNER_MAP_ID between 
						(SELECT PARTNER_MAP_ID FROM `k_vipcard` where ".$where." GROUP BY PARTNER_MAP_ID LIMIT ".$fiRow.",1) 
						and 
						(SELECT PARTNER_MAP_ID FROM `k_vipcard` where ".$where." GROUP BY PARTNER_MAP_ID LIMIT ".$liRow.",1)
						and CARD_STATUS != '8' group by PARTNER_MAP_ID";
						
						select sql_no_cache count(1) as TOTAL,sum(TRANS_AMT) as AMT from t_trace where TRACE_RETCODE='00' AND TRACE_REVERFLAG='0' AND TRANS_AMT>0 AND TRACE_VOIDFLAG='0' AND TRACE_REFUNDFLAG='0' AND TRANS_SUBID IN (43, 44, 39, 31, 32, 33, 38) group by SBRANCH_MAP_ID;
