...
class ApiserverController extends HomeController {
	...
	function queryH(){
		...
		$condition = array(
			'pieces.HOST_MAP_ID' => $host_map_id,
			'pieces.STATUS' => 0,
		);
		//夜间跑的商户
		$hour = intval(date('H'));echo $hour.'<br/>';
		if($hour>=22 || $hour<8){
			$condition['sh.MCC_CODE'] = array('in', '5813,7011,7911');
		}
		$count = $piecesModel->alias('pieces')
				->join('LEFT JOIN a_shop sh on sh.SHOP_MAP_ID=pieces.SHOP_MAP_ID')
				->where($condition)->count();
		echo $count.'<br/>';
		$rand = rand(0, $count-1);
		echo $rand;
		
		// $data = D($this->MShop)->findShop("SHOP_MAP_ID = 42 and SHOP_STATUS = 0", '*');
		
		// $rand = 251;
		// $pieces = $piecesModel->where($condition)->limit($rand,1)->select();
		// $pieces = $pieces[0];
		// dump($pieces);
		// $rand = 0;
		// $pieces = $piecesModel->alias('pieces')
				// ->field('pieces.*')
				// ->join('pai_db_jfb.a_shop sh on sh.SHOP_MAP_ID=pieces.SHOP_MAP_ID', 'LEFT')
				// ->where($condition)->limit($rand,1)->select();
		$pieces = $pieces[0];
		Add_LOG('Api_h', 'rand : '.$rand.' pieces : '.json_encode($pieces));
	}
	...
	//	$keyword = strtoupper(md5(strtoupper(md5($post['user_id'].'2016'))));
		$keyword = strtoupper(md5(strtoupper(md5($post['user_id'].'42803'))));
	...
	**/
	public function getlogindata() {
		...
		//会员
			...
			$jfbMemberModel = M("jfb_member", C("DB_PREFIX_JFB"), C("DB_DSN_JFB"));
			$jfbMember = $jfbMemberModel->query("select * from jfb_member_ where M_MOBILE='".$data['VIP_MOBILE']."'");
			if($jfbMember){
				$data['VIP_ID'] = $jfbMember['M_ID'];
			}
	...
	**/
	...
	public function shopsync(){
		...
		Add_LOG(CONTROLLER_NAME, json_encode(file_get_contents("php://input")));
		$post = array(
			'SHOP_MAP_ID'		=>	I('user_id'),		//商户ID
		);
		Add_LOG(CONTROLLER_NAME, json_encode($post));
		$sync_res = shop_sync_data($post['SHOP_MAP_ID'],1);
		if (!$sync_res) {
			$this->ajaxRets(1, '商户数据同步失败', $sync_res);
		}else{
			$this->ajaxRets(0, '商户数据同步成功', $sync_res);
		}
	}
	/*
	* 会员注册【13】【通了】
	**/
	public function addvip() {
		...
		$resdata = array(
			...
			'VIP_ADDRESS'		=>	'',
			'VIP_SEX'			=>	'',
			'VIP_EMAIL'			=>	'piccbill@bxh-group.com',
			'VIP_BIRTHDAY'		=>	'',
			'VIP_DONATE'		=>	'0',
			'VIP_DONATE_PER'	=>	'',
			'VIP_ID_M'			=>	'',
	...
	**/
	public function authvip() {
		...
		$resdata = array(
			...
			'VIP_SEX'			=>	$sex,
			'VIP_EMAIL'			=>	$post['VIP_EMAIL'] ? $post['VIP_EMAIL'] : 'piccbill@bxh-group.com',
	...
	**/
	public function updatevip() {
		...
		$resdata = array(
			...
			'VIP_EMAIL'			=>	$post['email'] ? $post['email'] : 'piccbill@bxh-group.com',
			'UPDATE_TIME'		=>	date('YmdHis'),
		);
