...
class SystemController extends HomeController {
	...
	public function user() {
		...
		if($post['submit'] == "user"){
			...
			if ($home['ROLE_NAME'] == 'OEM') {
				$where .= " and u.CHANNEL_MAP_ID = '".$home['CHANNEL_MAP_ID']."'";
				// $where .= " and u.ROLE_NAME = '".$home['ROLE_NAME']."'";
			}
			//分页
	...
	**/
	public function user_add() {
		...
		$while = "1 = 1";
		//判断是否为OEM和判断渠道
		if ($home['CHANNEL_MAP_ID'] != 0 && $home['ROLE_NAME'] == 'OEM') {
			$where .= " and ROLE_PID = '".$home['USER_ID']."'";
			$while .= " and CHANNEL_MAP_ID = '".$home['CHANNEL_MAP_ID']."'";
			$userModel = M('user');
			$user = $userModel->field('USER_NO')
							  ->where(array('CHANNEL_MAP_ID'=>$home['CHANNEL_MAP_ID'],'USER_STATUS'=>0))
							  ->order('ROLE_ID desc')
							  ->select();
			if (empty($user)) {
				$this->wrong('新增账号失败，请联系管理员');
			}
			$info['USER_NO'] = $user[0]['USER_NO'] + 1;
			// if ($info['USER_NO'] >= '0199') {
			// 	$info['USER_NO'] = '工号超限，请联系管理员';
			// }
			$this->assign('info',$info);
		}
		$role_list = D($this->MRole)->getRolelist($where, 'ROLE_ID,ROLE_NAME');	//除超管外		
		$this->assign('role_list', 			$role_list);
		$channel_list = M('channel')->where($while)->select();	//渠道列表		
	...
	**/
	public function user_edit() {
		...
		//判断是否为OEM和判断渠道
		if ($home['CHANNEL_MAP_ID'] != 0 && $home['ROLE_NAME'] == 'OEM') {
			if ($info['ROLE_NAME'] == 'OEM') {
				$where .= " and ROLE_NAME = '".$info['ROLE_NAME']."'";
			}else{
				$where .= " and ROLE_PID = '".$home['USER_ID']."'";
			}
			$while .= " and CHANNEL_MAP_ID = '".$home['CHANNEL_MAP_ID']."'";
