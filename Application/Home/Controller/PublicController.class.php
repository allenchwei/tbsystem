...
class PublicController extends HomeController {	
	...
	**/
    public function checklogin() {
		...
		$sedata = array(
			...
			'HOST_MAP_ID'		=>	$response['userinfo']['HOST_MAP_ID'],
			'CHANNEL_MAP_ID'	=>	$response['userinfo']['CHANNEL_MAP_ID'],
		);
		session('HOME', $sedata);
	...
	**/	
    public function ajaxgetcate() {
		...
		if (!empty($post['name'])) {
			$cateModel = M('cate_sf');
			if($post['level']==1){
				$condition['FIR_NAME'] = $post['name'];
				$group = 'SEC_NAME';
			}elseif($post['level']==2){
				$condition['FIR_NAME'] = $post['fir_name'];
				$condition['SEC_NAME'] = $post['name'];
				$group = 'THR_NAME';
			}
			$list = $cateModel->where($condition)->group($group)->order('id asc')->select();
			foreach($list as $val){
				$res .= '<option value="'.$val[$group].'">'.$val[$group].'</option>';
			}
		}else{
			$res = '';
		}
	...
	**/
	...
	public function ystestpay(){
		...
		$resdata = array(
			...
			'BankName'			=>	'中信银行大宁支行',							//银行
			'AccountNo'			=>	'6217730202345334',							//账户号	6217993300031483990	
			'AccountName'		=>	'阮超颖',						//账户名
			'Province'			=>	'',												//开户行所在省
			'City'				=>	'',												//开户行所在市
			'BankCode'			=>	setStrzero('302290031632', 12, '0', 'r'),	//银行行号
			'ExtraData'			=>	'',												//银行卡附加数据
		);
		//$ysConf = 'YinshengPay';
		//if($group['DKCO_MAP_ID'] == 3){
	...
	**/
	public function ysmorepay() {
		...
		$ip =  get_client_ip();
		$dkLockModel = M("dk_lock", DB_PREFIX_TRA, DB_DSN_TRA);
		$dkLockInfo = $dkLockModel->where(array('LOCK_NAME'=>'YSMOREPAY'))->find();
		if(empty($dkLockInfo)){
			echo json_encode(array('status'=>204,'message'=>'锁定表不存在不能执行代扣'));exit;
		}elseif($dkLockInfo['LOCK']==1){
			echo json_encode(array('status'=>201,'message'=>'锁定状态不能执行代扣'));exit;
		}elseif($dkLockInfo['LOCK_IP']!=$ip){
			echo json_encode(array('status'=>202,'message'=>'IP地址有误不能执行代扣'));exit;
		}elseif($dkLockInfo['LOCK_DATE']==date('Ymd')){
			echo json_encode(array('status'=>203,'message'=>'每天只能执行一次代扣'));exit;
		}
		// exit;
		$dkLockModel->where(array('LOCK_NAME'=>'YSMOREPAY','LOCK'=>'0'))->save(array('LOCK'=>'1','LOCK_DATE'=>date('Ymd')));
		...
		//查询
		$sellist = D($this->TDkls)->getDklsgrouplist("DKCO_MAP_ID = 3 and DK_FLAG != 0 and DK_ORDER_ID > 0", 'DK_ORDER_ID', '', 'DK_ORDER_ID', 'DK_ORDER_ID desc');
		if(!empty($sellist)) {
			for($i = 0; $i<count($sellist); $i++){
				$group = array();
				$field = 'DKCO_MAP_ID,DK_ORDER_ID,DK_DATE,sum(DK_AMT) as DK_AMT';
				$group = D($this->TDkls)->findNewsDkls("DKCO_MAP_ID = 3 and DK_ORDER_ID = '".$sellist[$i]['DK_ORDER_ID']."'", $field);
				...
				//银盛
				elseif($group['DKCO_MAP_ID'] == 1 || */if($group['DKCO_MAP_ID'] == 3){
					//组装数据			
					$resdata = array(
						'Ver'			=>	'1.0',											//版本号
						'MsgCode'		=>	'S5001',										//报文编号
				...
				if($state == '00') {
					...
					//分摊列表
					$fentanlist	= D($this->TDkls)->getNewsDklslist("DKCO_MAP_ID = 3 and DK_ORDER_ID = '".$group['DK_ORDER_ID']."'", 'DK_ID');
					...
					foreach($fentanlist as $key=>$val){
						...
						D($this->TDkls)->updateDkls("DKCO_MAP_ID = 3 and DK_ID = '".$val['DK_ID']."'", $updata);				
					}
		...
		//代扣
		$dklist = D($this->TDkls)->getDklsgrouplist("DKCO_MAP_ID = 3 and DK_FLAG != 0", 'BANKACCT_NO,SHOP_NO', '', 'BANKACCT_NO');
		if(empty($dklist)) {
			echo '无代扣数据！';
			exit;
		}
		...
		for($j = 0; $j<count($dklist); $j++){
			...
			$group = D($this->TDkls)->findNewsDkls("DKCO_MAP_ID = 3 and DK_FLAG != 0 and BANKACCT_NO = '".$dklist[$j]['BANKACCT_NO']."'", $field);
			if($oldShopNo == $dklist[$j]['SHOP_NO']){
				continue;
			}
			$oldShopNo = $dklist[$j]['SHOP_NO'];
			//检查必须项
			if(empty($group['BANKACCT_NO']) || empty($group['BANKACCT_NAME']) || empty($group['DK_AMT'])){
				...
				D($this->TDkls)->updateDkls("DKCO_MAP_ID = 3 and DK_FLAG != 0 and BANKACCT_NO = '".$group['BANKACCT_NO']."'", $updata);
				continue;
			}
			...
			if($state == '00') {
				...
				//分摊列表
				$fentanlist	= D($this->TDkls)->getNewsDklslist("DKCO_MAP_ID = 3 and DK_FLAG != 0 and BANKACCT_NO = '".$group['BANKACCT_NO']."'", 'DK_ID');
		...
		echo '查询、批量代扣执行完成！';
		$dkLockModel->where(array('LOCK_NAME'=>'YSMOREPAY','LOCK'=>'1'))->save(array('LOCK'=>'0'));
		exit;	
	}
