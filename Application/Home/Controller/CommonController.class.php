...
class CommonController extends HomeController {
	...
	public function channel_add() {
		...
		if($post['submit'] == "channel_add") {
			...
			$resdata = array(
				...
				'KEYT'		=>	$post['KEYT'],
				'HOST_MAP_ID'		=>	$post['HOST_MAP_ID'],
				'HOST_MAP_ID_KJ'		=>	$post['HOST_MAP_ID_KJ'],
				'HOST_MAP_ID_WG'		=>	$post['HOST_MAP_ID_WG'],
				'HOST_MAP_ID_DK'		=>	$post['HOST_MAP_ID_DK'],
				'MINUS_FEE'		=>	$post['MINUS_FEE'],
				'DF_FEE'		=>	($post['DF_FEE'] * 100),
				'BRANCH_MAP_ID'		=>	0,
	...
	**/
	public function channel_show() {
		...
		//通道列表
		$host_list = M('host')->select();
		$this->assign ('host_list', $host_list);		//host_list
		$this->assign ('channel_status', C('CHANNEL_STATUS'));		//渠道状态
	...
	**/
	public function channel_edit() {
		...
		if($post['submit'] == "channel_edit") {
			...
			//支付通道
			$host_map_list = implode(',',$_POST['HOST_MAP_LIST']);
			$host_map_list = $host_map_list?$host_map_list:'';
			//代付通道
			$host_map_dflist = implode(',',$_POST['HOST_MAP_DFLIST']);
			$host_map_dflist = $host_map_dflist?$host_map_dflist:'';
			$home = session('HOME');
			if(empty($post['KEYT']) && $post['MCH_ID']){
				$post['KEYT'] = strtoupper(hash_hmac('md5', $post['MCH_ID'], date('YmdHis').rand(100,999)));
			}
			if(empty($post['DATA_KEY']) && $post['MCH_ID']){
				$post['DATA_KEY'] = strtoupper(hash_hmac('md5', $post['MCH_ID'], 'DATA_KEY'.date('YmdHis').rand(100,999)));
			}
			//组装数据
			$resdata = array(
				'MCH_ID'		=>	$post['MCH_ID'],
				'KEYT'		=>	$post['KEYT'],
				'DATA_KEY'		=>	$post['DATA_KEY'],
				'HOST_MAP_ID'		=>	$post['HOST_MAP_ID'],
				'HOST_MAP_ID_KJ'		=>	$post['HOST_MAP_ID_KJ'],
				'HOST_MAP_ID_WG'		=>	$post['HOST_MAP_ID_WG'],
				'HOST_MAP_ID_DK'		=>	$post['HOST_MAP_ID_DK'],
				...
				'MOBILE'			=>	$post['MOBILE'],
				'EMAIL'				=>	$post['EMAIL'],
				'HOST_MAP_LIST'		=>	$host_map_list,
				'HOST_MAP_DFLIST'		=>	$host_map_dflist,
				'CREATE_USERID'		=>	$home['USER_ID'],
				'CREATE_USERNAME'	=>	$home['USER_NAME'],
				'CREATE_TIME'		=>	date('YmdHis')
			);
		...
		$this->assign ('channel_status', C('CHANNEL_STATUS'));		//渠道状态
		$this->assign ('info', $info);								//数据信息
		$this->assign ('host_map_list', explode(',',$info['HOST_MAP_LIST']));								//数据信息
		$this->assign ('host_map_dflist', explode(',',$info['HOST_MAP_DFLIST']));								//数据信息
		$this->assign ('ccls_info', $ccls_info);					//结算方式信息
	...
	* 代付公司管理
	**/
	public function dfco() {
		$post = I('post');
		if($post['submit'] == "dfco"){
			$where = "DFCO_STATUS = 0";
			//代扣公司名称
			if($post['DFCO_NAME']) {
				$where .= " and DFCO_NAME like '%".$post['DFCO_NAME']."%'";
			}
			//状态
			if($post['DFCO_STATUS'] != '') {
				$where .= " and DFCO_STATUS = '".$post['DFCO_STATUS']."'";
			}
			//分页
			$dfcoModel = M('dfco');
			$count = $dfcoModel->where($where)->count();
			$p 	   = new \Think\Page($count, C('PAGE_COUNT'));
			$list  = $dfcoModel->where($where)->limit($p->firstRow.','.$p->listRows)->select();
			//分页参数
			$this->assign ( 'totalCount', 	$count );
	       	$this->assign ( 'numPerPage', 	$p->listRows );
	       	$this->assign ( 'currentPage', 	!empty($_REQUEST[C('VAR_PAGE')]) ? $_REQUEST[C('VAR_PAGE')] : 1);	
		...
		$this->assign('dfco_fee_flag',		C('DKCO_FEE_FLAG'));	//代扣公司收费标准
		$this->assign('dfco_status',		C('DKCO_STATUS'));		//代扣公司状态
		\Cookie::set ('_currentUrl_', 		__SELF__);
		$this->display();
	}
	/*
	* 代付公司管理 添加
	**/
	public function dfco_add() {
		$post = I('post');
		if($post['submit'] == "dfco_add") {
			//验证
			if(empty($post['DFCO_NAME']) || empty($post['CITY_NO']) || empty($post['ADDRESS']) || empty($post['ZIP']) || empty($post['MANAGER']) || empty($post['MOBILE']) || empty($post['DFCO_DF_T']) || empty($post['DFCO_DF_LOWAMT'])){
				$this->wrong("缺少必填项数据！");
			}
			//组装数据
			$resdata = array(
				'DFCO_NAME'			=>	$post['DFCO_NAME'],
				'DFCO_STATUS'		=>	$post['DFCO_STATUS'],
				'DFCO_FEE_FLAG'		=>	$post['DFCO_FEE_FLAG'],
				'DFCO_FEE_FIX'		=>	$post['DFCO_FEE_FLAG'] == 0 ? setMoney($post['DFCO_FEE_FIX'], '2') : '',
				'DFCO_FEE_PER'		=>	$post['DFCO_FEE_PER'],
				'DFCO_DF_T'			=>	$post['DFCO_DF_T'],
				'DFCO_DF_WD'		=>	$post['DFCO_DF_WD'],
				'DFCO_DF_LOWAMT'	=>	setMoney($post['DFCO_DF_LOWAMT'], '2'),
				'CITY_NO'			=>	$post['CITY_NO'],
				'ADDRESS'			=>	$post['ADDRESS'],
				'ZIP'				=>	$post['ZIP'],
				'MANAGER'			=>	$post['MANAGER'],
			...
			$dfcoModel = M('dfco');
			$res = $dfcoModel->add($resdata);
			if(!$res){
				$this->wrong('代付公司添加失败');
			}
			$this->right('代付公司添加成功', 'closeCurrent', \Cookie::get ( '_currentUrl_' ), $_REQUEST['navTabId']);
		}
		$info['DFCO_FEE_FLAG'] = 1;
		$this->assign('info',		$info);	
		$this->assign('dfco_fee_flag',		C('DKCO_FEE_FLAG'));	//代扣公司收费标准
		$this->assign('dfco_status',		C('DKCO_STATUS'));		//代扣公司状态
		$this->assign('dfco_df_wd',			C('DKCO_DK_WD'));		//代扣公司工作日
		$this->display();
	}
	/*
	* 代付公司管理 详情
	**/
	public function dfco_show() {		
		$id = $_REQUEST['id'];
		if(empty($id)){
			$this->wrong('缺少参数！');
		}
		$dfcoModel = M('dfco');
		$info = $dfcoModel->where(array('DFCO_MAP_ID'))->find();
		if(empty($info)){
			$this->wrong("参数数据出错！");
		}
		$this->assign('dfco_fee_flag',		C('DKCO_FEE_FLAG'));	//代扣公司收费标准
	...
	* 代付公司管理 修改
	**/
	public function dfco_edit() {		
		$post = I('post');
		if($post['submit'] == "dfco_edit") {
			//验证
			if(empty($post['DFCO_NAME']) || empty($post['CITY_NO']) || empty($post['ADDRESS']) || empty($post['ZIP']) || empty($post['MANAGER']) || empty($post['MOBILE']) || empty($post['DFCO_DF_T']) || empty($post['DFCO_DF_LOWAMT'])){
				$this->wrong("缺少必填项数据！");
			}
			if(($post['DFCO_FEE_FLAG']==0 && empty($post['DFCO_FEE_FIX'])) || ($post['DFCO_FEE_FLAG']==1 && empty($post['DFCO_FEE_PER']))) {
				$this->wrong("请输入收费标准！");
			}
			//组装数据
			$resdata = array(
			...
			$dfcoModel = M('dfco');
			$res = $dfcoModel->where(array('DFCO_MAP_ID'=>$post['DFCO_MAP_ID']))->save($resdata);
			if(!$res){
				$this->wrong('代付修改失败');
			}
			$this->right('代付公司修改成功', 'closeCurrent', \Cookie::get ( '_currentUrl_' ), $_REQUEST['navTabId']);
		}
		...
		$dfcoModel = M('dfco');
		$info = $dfcoModel->where(array('DFCO_MAP_ID'=>$id))->find();
		if(empty($info)){
			$this->wrong("参数数据出错！");
		}
		$this->assign('dfco_fee_flag',		C('DKCO_FEE_FLAG'));	//代扣公司收费标准
	...
	public function ad() {
		...
		if($post['submit'] == "ad"){
			...
			if($post['CHANNEL_MAP_ID']) {
				$where .= " and ad.CHANNEL_MAP_ID=".$post['CHANNEL_MAP_ID'];
			}
			if($home['CHANNEL_MAP_ID']>0){
				$where .= " and ad.CHANNEL_MAP_ID=".$home['CHANNEL_MAP_ID'];
			}
			//名称
			if($post['AD_NAME']) {
				$where .= " and ad.AD_NAME like '%".$post['AD_NAME']."%'";
			}
			//状态
			if($post['STATUS'] != '') {
				$where .= " and ad.STATUS = '".$post['STATUS']."'";
			}
			//分页
			$adModel = M('ad');
			$count = $adModel->alias('ad')->where($where)->count();
			$p 	   = new \Think\Page($count, C('PAGE_COUNT'));
			$list  = $adModel->alias('ad')
					->join("pai_db_jfb.a_channel ch on ch.CHANNEL_MAP_ID=ad.CHANNEL_MAP_ID", "LEFT")
		...
		$condition = array('CHANNEL_STATUS'=>0);
		if($home['CHANNEL_MAP_ID']>0){
			$condition['CHANNEL_MAP_ID'] = $home['CHANNEL_MAP_ID'];
		}
		$channelModel = M("channel");
		$channel_list = $channelModel->where($condition)->select();
		$this->assign('channel_list',		$channel_list);		//渠道列表
		\Cookie::set ('_currentUrl_', 		__SELF__);
		$this->assign('ad_type_list',		C('AD_TYPE_LIST'));		//广告分类列表
		$this->assign('status',		C('DKCO_STATUS'));		//代扣公司状态
		$this->display();
	}
	...
	public function ad_add() {
		$post = I('post');
		$home = session('HOME');
		if($post['submit'] == "ad_add") {
			//验证
			if(empty($post['AD_NAME']) || empty($post['AD_TYPE']) || empty($post['AD_PIC'])){
				$this->wrong("缺少必填项数据！");
			}
			//组装数据
			$resdata = array(
	...
	public function ad_edit() {
		...
		if($post['submit'] == "ad_edit") {
			...
			$adModel = M('ad');
			$res = $adModel->where(array('AD_ID'=>$post['AD_ID']))->save($resdata);
			if(!$res){
				$this->wrong('修改失败');
			}
			$this->right('修改成功', 'closeCurrent', \Cookie::get ( '_currentUrl_' ), $_REQUEST['navTabId']);
		...
		$adModel = M('ad');
		$info = $adModel->where(array('AD_ID'=>$id))->find();
		if(empty($info)){
			$this->wrong("参数数据出错！");
		}
		$channelModel = M("channel");
	...
	public function downmch(){
        $post = I('post');
        if($post['submit'] == "downmch"){
            $where = "downmch.STATUS = 1";
            //类型
            // if($post['DOWNMCH_STATUS']) {
            //     $where .= " and STATUS =".$post['DOWNMCH_STATUS'];
            // }
            if ($post['SHOP_MAP_ID']) {
            	$where .= " and downmch.SHOP_MAP_ID =".$post['SHOP_MAP_ID'];
            }
            //分页
            $count = D($this->MDownmch)->countDownmch($where);
            $where .= " and piece.STATUS = 0";
            $p 	   = new \Think\Page($count, C('PAGE_COUNT'));
            // $Downlist  = D($this->MDownmch)->getDownmchlist($where, '*', $p->firstRow.','.$p->listRows);
            // // var_dump($list);
            $piece = M('shop_intopieces');
            $list = $piece->alias('piece')
							->join('a_downmch downmch on downmch.PIECE_ID = piece.ID')
							// ->join('a_channel channel on channel.HOST_MAP_ID = piece.HOST_MAP_ID')
							->join('a_host host on host.HOST_MAP_ID = piece.HOST_MAP_ID')
							->where($where)
							->limit($p->firstRow.','.$p->listRows)
							->field('downmch.SHOP_MAP_ID,host.HOST_NAME,host.HOST_MAP_ID,downmch.MCH_ID,downmch.KEY,downmch.FLAG,downmch.PIECE_ID,downmch.ID')
							->order('downmch.ID desc')
							->select();
			// var_dump($list);
            //分页参数
            $this->assign ( 'totalCount', 	$count );
            ...
            $this->assign ( 'postdata', 	$post );
            $this->assign ( 'list', 		$list );
            $this->assign ('downmch_status', C('DOWNMCH_STATUS'));
            $this->assign ('downmch_flag', C('DOWNMCH_FLAG'));
        }
        $this->assign ('downmch_status', C('DOWNMCH_STATUS'));		//渠道状态
        \Cookie::set ('_currentUrl_', 		__SELF__);
        $this->display();
    }
	...
	* 下游商户管理 添加
	**/
	public function downmch_add() {
		$post = I('post');
		if($post['submit'] == "downmch_add") {
			//验证
			if(empty($post['SHOP_MAP_ID']) || empty($post['MCH_ID']) || empty($post['KEY']) || $post['FLAG'] == '' || empty($post['PIECE_ID'])){
				$this->wrong("缺少必填项数据！");
			}
			//组装数据
			$resdata = array(
			...
			$m = M();
			$m->startTrans();	//启用事务
			$res = D($this->MDownmch)->addDownmch($resdata);
			if($res['state'] != 0){
				$this->wrong($res['msg']);
			}
			$m->commit();			//成功则提交
	...
	* 下游商户管理 修改
	**/
	public function downmch_edit(){
		$post = I('post');
		if($post['submit'] == "downmch_edit") {
			//验证
			if(empty($post['SHOP_MAP_ID']) || empty($post['MCH_ID']) || empty($post['KEY']) || $post['FLAG'] == '' || empty($post['PIECE_ID'])){
				$this->wrong("缺少必填项数据！");
			}
			...
			$m = M();
			$m->startTrans();	//启用事务
			$res = D($this->MDownmch)->saveDownmch($where,$resdata);
			if($res['state'] != 0){
				$this->wrong($res['msg']);
			}
			$m->commit();			//成功则提交
		...
		$downmch = M('downmch');
		$info = $downmch->where('ID='.$id)->find();
		$this->assign ('info', $info);						//数据信息
		$this->assign ('downmch_flag', C('DOWNMCH_FLAG'));
		$this->display('downmch_add');
	}

	/*
	* 下游商户管理 删除
	**/
	public function downmch_del(){
		$id = $_REQUEST['id'];
		if(empty($id)){
			$this->wrong('缺少参数');
		}
		$downmchModel = M('downmch');
		$res = $downmchModel->where(array('ID'=>$id))->save(array('STATUS'=>0));
		if(!$res){
			$this->wrong('删除失败');
		}
	...
	**/
	public function chra() {
		...
		if($post['submit'] == "chra"){
			...
			//渠道名称
			if($post['CHANNEL_NAME']) {
				$where .= " and ch.CHANNEL_NAME like '%".$post['CHANNEL_NAME']."%'";
			}
			//通道名称
			if($post['HOST_NAME']) {
			...
			$chrModel = M('chr');
			//分页
			$count = $chrModel->alias('chr')
					->join('pai_db_jfb.a_channel ch on ch.CHANNEL_MAP_ID=chr.CHANNEL_MAP_ID', 'LEFT')
					->join('pai_db_jfb.a_host ho on ho.HOST_MAP_ID=chr.HOST_MAP_ID', 'LEFT')
					->where($where)->count();
			$p 	   = new \Think\Page($count, C('PAGE_COUNT'));
			$list = $chrModel->alias('chr')
					->field('ch.CHANNEL_NAME,ho.HOST_NAME,chr.*')
					->join('pai_db_jfb.a_channel ch on ch.CHANNEL_MAP_ID=chr.CHANNEL_MAP_ID', 'LEFT')
					->join('pai_db_jfb.a_host ho on ho.HOST_MAP_ID=chr.HOST_MAP_ID', 'LEFT')
					->where($where)
					->order('chr.CHANNEL_MAP_ID,chr.HOST_MAP_ID,chr.SHOP_GRADE')
					->limit($p->firstRow.','.$p->listRows)
					->select();
			//分页参数
			$this->assign ( 'totalCount', 	$count );
	...
	* 渠道通道管理 添加
	**/
	public function chr_add() {
		$post = I('post');
		if($post['submit'] == "chr_add") {
			//验证
			if(empty($post['HOST_MAP_ID']) || empty($post['CHANNEL_MAP_ID']) || empty($post['PER_FEE']) || empty($post['SHOP_GRADE'])){
				$this->wrong("缺少必填项数据！");
			}
			$home = session('HOME');
			//组装数据
			$resdata = array(
				'CHANNEL_MAP_ID'		=>	$post['CHANNEL_MAP_ID'],
				'HOST_MAP_ID'		=>	$post['HOST_MAP_ID'],
				'PER_FEE'		=>	setMoney($post['PER_FEE'], '2'),
	...
	* 渠道通道管理 编辑
	**/
	public function chr_edit() {
		$post = I('post');
		if($post['submit'] == "chr_edit") {
			//验证
			if(empty($post['CHR_ID']) || empty($post['HOST_MAP_ID']) || empty($post['CHANNEL_MAP_ID']) || empty($post['PER_FEE']) || empty($post['SHOP_GRADE'])){
				$this->wrong("缺少必填项数据！");
			}
			$home = session('HOME');
			//组装数据
			...
			$m = M();
			$m->startTrans();	//启用事务
			$res = M('chr')->where(array('CHR_ID'=>$post['CHR_ID']))->save($resdata);
			if(!$res){
				$this->wrong('修改失败');
			}
			$m->commit();			//成功则提交
		...
		//渠道通道信息
		$info = M('chr')->where(array('CHR_ID'=>$id))->find();
		//通道列表
		$host_list = M('host')->select();
		//渠道列表
		$channel_list = M('channel')->select();
		$this->assign ('info', $info);		//渠道通道信息
		$this->assign ('host_list', $host_list);		//通道列表
		$this->assign ('channel_list', $channel_list);		//渠道列表
		$this->assign ('channel_status', C('CHANNEL_STATUS'));		//渠道状态
		$this->display('chr_add');
	}
}?>