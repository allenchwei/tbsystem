...
class JfbcardController extends HomeController {
	...
	**/
	public function carddata_export() {
		...
		//分公司
		if($post['bid'] != '') {
			$where .= " and vc.BRANCH_MAP_ID = '".$post['bid']."'";
		}
		//合作伙伴公司
		if($post['pid'] != '') {
			$where .= " and vc.PARTNER_MAP_ID in(".$post['pid'].")";
		}
		//卡套餐
		if($post['CARD_P_MAP_ID'] != '') {
			$where .= " and vc.CARD_P_MAP_ID = '".$post['CARD_P_MAP_ID']."'";
	...
	**/
	public function statiscard() {
		...
		if($post['submit'] == "statiscard"){
			$where = "1=1";
			//===优化统计===
			$falv = $ajax == 'loading' ? $ajax_soplv : filter_data('falv');	//列表查询
			//===结束=======
			//分公司
			if($falv['bid'] != '') {
			...
			if($post['CARD_P_MAP_ID'] != '') {
				$where .= " and CARD_P_MAP_ID = '".$post['CARD_P_MAP_ID']."'";
				$stock_where1 .= " and CARD_P_MAP_ID = '".$post['CARD_P_MAP_ID']."'";
			}			
			//卡片状态
			if($post['CARD_STATUS'] != '') {
				$where .= " and CARD_STATUS = '".$post['CARD_STATUS']."'";
				$stock_where2 .= " and CARD_STATUS = '".$post['CARD_STATUS']."'";
			}else{
				$where .= " and CARD_STATUS != '8'";
			}
			//分组选择
		//	if($falv['pid'] != '') {
				$group = 'PARTNER_MAP_ID';
		//	}else{
		//		$group = 'BRANCH_MAP_ID,PARTNER_MAP_ID';
		//	}				
			//===优化统计===
			if($ajax == 'loading'){
				$total1    = D($this->GVipcard)->countVipcard($where);
				$total2    = D($this->GVipcard)->countVipcard($where." and CARD_STATUS = '1'");	
				$resdata = array(
					'zongke'	=>	$total1,
					'kucun'		=>	$total2,
					'yishou'	=>	$total1 - $total2,
				);
				$this->ajaxReturn($resdata);
			}
			//===结束=======
			...
			$fiRow = ($_REQUEST[C('VAR_PAGE')] ? $_REQUEST[C('VAR_PAGE')]-1 : 0) * C('PAGE_COUNT');
			$liRow = C('PAGE_COUNT');
			$list  = D($this->GVipcard)->groupVipcardlist($where, 'count(CARD_NO) as CARD_COUNT,BRANCH_MAP_ID,PARTNER_MAP_ID,CARD_P_MAP_ID', $fiRow.','.$liRow, $group, 'CARD_COUNT desc');
			if(!empty($list)) {
				foreach($list as $key => $value) {
					if(!empty($value['PARTNER_MAP_ID'])) {
						$stock_where = 'PARTNER_MAP_ID = '.$value['PARTNER_MAP_ID'].$stock_where1.$stock_where2;
					}else{
						$stock_where = 'BRANCH_MAP_ID = '.$value['BRANCH_MAP_ID'].' and PARTNER_MAP_ID = 0'.$stock_where1.$stock_where2;
					}
					if(empty($stock_where2)) {
						$stock_where3 = ' and CARD_STATUS = 1';
					}
					$list[$key]['STOCK_COUNT'] = D($this->GVipcard)->countVipcard($stock_where.$stock_where3);
				}
			}
