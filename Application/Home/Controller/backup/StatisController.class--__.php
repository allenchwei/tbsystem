	...
	**/
	public function viptj() {
		...
		if($post['submit'] == "viptj"){
			...
			//===优化统计===
			if($ajax == 'loading'){			
				$count_sql  = 'SELECT COUNT(PARTNER_MAP_ID) AS CNT,BRANCH_MAP_ID,PARTNER_MAP_ID FROM `k_vip` WHERE '.$where;
			...
			$liRow 	   = C('PAGE_COUNT');			
			$list_sql  = 'SELECT COUNT(PARTNER_MAP_ID) AS CNT,BRANCH_MAP_ID,PARTNER_MAP_ID FROM `k_vip` WHERE '.$where.' GROUP BY PARTNER_MAP_ID ORDER BY CNT DESC LIMIT '.$fiRow.','.$liRow;
