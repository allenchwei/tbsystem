...
class MemberController extends HomeController {
	...
	**/
	public function vbus() {
		...
		if($post['submit'] == "vbus"){
			...
			//身份证号
			if($post['VIP_IDNO'] != '') {
				$where .= " and VIP_IDNO like '".$post['VIP_IDNO']."%'";
			}
			//手机号
			if($post['VIP_MOBILE'] != '') {
				$where .= " and VIP_MOBILE like '".$post['VIP_MOBILE']."%'";
			}
			//实名认证
			...
			//会员卡号
			if($post['CARD_NO']) {
				$where .= " and CARD_NO like '".$post['CARD_NO']."%'";
			}
			//会员名称
			if($post['VIP_NAME']) {
			...
			$list  = D($this->GVip)->getNewsViplist($where, '*', $p->firstRow.','.$p->listRows);
			echo M('vip',DB_PREFIX_GLA, DB_DSN_GLA)->getLastSql();
	...
	**/
	public function vbus_add() {
		...
		if($post['submit'] == "vbus_add") {
			...
			//验证
			if(empty($post['VIP_NAME']) || empty($post['VIP_IDNO']) || empty($post['VIP_BIRTHDAY']) || empty($post['VIP_MOBILE']) || empty($post['VIP_CITY']) || empty($post['VIP_ADDRESS']) || empty($post['VIP_EMAIL'])){
				$this->wrong("请完善基础信息！");
			}
			...
			//检测证件号
			$findvip1 = D($this->GVip)->findVip("VIP_IDNOTYPE = '".$post['VIP_IDNOTYPE']."' and VIP_IDNO = '".$post['VIP_IDNO']."'");
			if(!empty($findvip1)){
				$this->wrong("该证件号已注册！");
			}
			//检测手机号
			$findvip2 = D($this->GVip)->findVip("VIP_MOBILE = '".$post['VIP_MOBILE']."'");
			if(!empty($findvip2)){
				$this->wrong("该手机号已注册！");
			}
			//检查性别是否合法
			...
			//检查卡号是否重复
			$findvip3 = D($this->GVip)->findVip("CARD_NO = '".$post['CARD_NO']."'");
			if(!empty($findvip3)){
				$this->wrong("该卡号已注册！");
			}
			//卡号信息，检测卡号、验证码，
			$findvipcard = D($this->GVipcard)->findVipcard("BRANCH_MAP_ID = '".$post['BRANCH_MAP_ID']."' and PARTNER_MAP_ID = '".$post['PARTNER_MAP_ID']."' and CARD_NO = '".$post['CARD_NO']."' and CARD_STATUS=1 and CARD_CHECK = '".$post['CARD_CHECK']."'");
			...
			$data = array(
				...
				'mId' 	 		 => $res['VIP_ID'],						//(会员ID)
				'mCId' 	 		 => $resdata['CARD_NO'] ? $resdata['CARD_NO'] : $resdata['VIP_MOBILE'],	//(卡号)
	...
	**/
	public function vbus_show() {
		...
		$info = D($this->GVip)->findVip("VIP_ID = '".$id."'");
		if(empty($info)){
			$this->wrong("参数数据出错！");
		}
		...
		$lapdata = D($this->GLap)->findLap("ACCT_NO = '".setStrzero($info['VIP_ID'], 9)."'");
                echo M('lap', DB_PREFIX_GLA, DB_DSN_GLA)->getLastSql();
		...
		$vipcard  = D($this->GVipcard)->findVipcard("VIP_ID = '".$info['VIP_ID']."' and CARD_STATUS = 0", 'CARD_P_MAP_ID');
                echo M('vipcard', DB_PREFIX_GLA, DB_DSN_GLA)->getLastSql();
		if(!empty($vipcard)){
	...
	**/
	public function vinfo_edit() {
		...
		if($post['submit'] == "vinfo_edit") {
			...
			//检测证件号
			$findvip1 = D($this->GVip)->findVip("VIP_ID != '".$post['VIP_ID']."' and VIP_IDNOTYPE = '".$post['VIP_IDNOTYPE']."' and VIP_IDNO = '".$post['VIP_IDNO']."'");
			if(!empty($findvip1)){
				$this->wrong("该证件号已注册！");
			}
			//检测手机号
			$findvip2 = D($this->GVip)->findVip("VIP_ID != '".$post['VIP_ID']."' and VIP_MOBILE = '".$post['VIP_MOBILE']."'");
			if(!empty($findvip2)){
				$this->wrong("该手机号已注册！");
			}
			//检查性别是否合法
			...
			$data = array(
				...
				'mCId' 	 		 => $post['CARD_NO'] ? $post['CARD_NO'] : $resdata['VIP_MOBILE'],	//(卡号)
				'mName' 		 => $resdata['VIP_NAME'],				//(会员姓名)
				'mIdentityType'	 => $resdata['VIP_IDNOTYPE'],			//(证件类型)
				'mIdentityId' 	 => $resdata['VIP_IDNO'],				//(证件号)
				'mBirthday'		 => $resdata['VIP_BIRTHDAY'],			//(会员生日)
	...
	**/
	public function vfcard() {
		...
		if($post['submit'] == "vfcard"){
			$where = "(VIP_CARD_FLAG ='-' or VIP_CARD_FLAG ='1')";
			//归属
			$getlevel = filter_data('plv');	//列表查询
			$post['BRANCH_MAP_ID']  = $getlevel['bid'];
			$post['PARTNER_MAP_ID'] = $getlevel['pid'];
	...
	**/
	public function vfcard_bind() {
		...
		if($post['submit'] == "vfcard_bind") {
			...
			//同步修改会员数据
			$info = D($this->GVip)->findVip("VIP_ID = '".$post['VIP_ID']."'");
			$url = VIP_PUSH_URL.'api/open/synchronize/member/modify';
			$data = array(
				'token' 	 	 => strtoupper(md5(strtoupper(md5($post['VIP_ID'].'2')))),		//(签名验证)
				'mId' 	 		 => $post['VIP_ID'],					//(会员ID)
	...
	**/
	public function vtcard() {
		...
		if($post['submit'] == "vtcard"){
			$where = "(VIP_CARD_FLAG ='2' or VIP_CARD_FLAG ='3')";
			//归属
			$getlevel = filter_data('plv');	//列表查询
			$post['BRANCH_MAP_ID']  = $getlevel['bid'];
			$post['PARTNER_MAP_ID'] = $getlevel['pid'];
	...
	**/
	public function vtcard_exch() {
		...
		if($post['submit'] == "vtcard_exch") {
			...
			$res_del  = D($this->GVipcard)->delVipcard("VIP_ID = '".$post['VIP_ID']."'");
			if($res_del['state'] != 0){
				$m->rollback();	//回滚
				$this->wrong('换卡失败！');
			}
	...
	**/
	public function shopapply() {
		...
		/*	//创建日期	开始
			...
			$p 	   = new \Think\Page($count, C('PAGE_COUNT'));
			$list  = D($this->MShopapply)->getShopapplylist($where, '*', $p->firstRow.','.$p->listRows);		
