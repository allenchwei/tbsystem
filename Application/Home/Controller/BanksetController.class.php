...
class BanksetController extends HomeController {
	...
	* 代付时间管理
	**/
	public function dfdate() {
		$post = I('post');
		if($post['submit'] == "dfdate"){
			$where = "1=1";
			//所属分支
			if($post['DFCO_MAP_ID']) {
				$where .= " and dfd.DFCO_MAP_ID = ".$post['DFCO_MAP_ID'];
			}
			//分页
			$count = M('dfdate')->alias('dfd')->where($where)->count();
			$p 	   = new \Think\Page($count, C('PAGE_COUNT'));
			$list  = M('dfdate')->alias('dfd')->field('dfd.*,dfco.DFCO_NAME')
					->join('pai_db_jfb.a_dfco dfco on dfco.DFCO_MAP_ID=dfd.DFCO_MAP_ID','LEFT')
		...
		$this->assign('dfco_list',		$dfco_list);	//代付公司列表
		$this->assign('user_status',		C('USER_STATUS'));	//分支机构级别
		\Cookie::set ('_currentUrl_', 		__SELF__);
		$this->display();
	}
	...
	* 代付时间管理 添加
	**/
	public function dfdate_add() {
		$post = I('post');
		if($post['submit'] == "dfdate_add") {
			//验证
			if($post['START_TIME']!='' || $post['END_TIME']!='' || empty($post['DFCO_MAP_ID']) || empty($post['RES'])){
				$this->wrong("缺少必填项数据！");
			}
			...
			$count = M('dfdate')->where(array('DFCO_MAP_ID'=>$post['DFCO_MAP_ID'], 'START_TIME'=>$post['START_TIME'], 'END_TIME'=>$post['END_TIME']))->count();
			if($count > 0){
				$this->wrong("该时间段已经存在！");
			}			
			//组装数据
			$resdata = array(
				...
				'DFCO_MAP_ID'		=> 	$post['DFCO_MAP_ID'],
				'RES'			=>	$post['RES']
			);
			$res = M('dfdate')->add($resdata);
			if(!$res){
				$this->wrong('添加失败');
			}
		...
		$dfco_list = $dfcoModel->select();
		$this->assign('dfco_list',		$dfco_list);	//代付公司列表
		$this->assign('time_list',		$time_list);	//时间列表
		$this->display();
	}
	/*
	* 代付时间管理 修改
	**/
	public function dfdate_edit() {
		$post = I('post');
		if($post['submit'] == "dfdate_edit") {
			//验证
			if($post['START_TIME']!='' || $post['END_TIME']!='' || empty($post['DFCO_MAP_ID']) || empty($post['RES'])){
				$this->wrong("缺少必填项数据！");
			}
			...
			$count = M('dfdate')->where(array('DATE_ID'=>array('neq', $post['DATE_ID']), 'DFCO_MAP_ID'=>$post['DFCO_MAP_ID'], 'START_TIME'=>$post['START_TIME'], 'END_TIME'=>$post['END_TIME']))->count();
			if($count > 0){
				$this->wrong("该时间段已经存在！");
			}
			//组装数据
			...
			$res = M('dfdate')->where(array('DATE_ID'=>$post['DATE_ID']))->save($resdata);
			if(!$res){
				$this->wrong('修改失败');
			}
		...
		$info = M('dfdate')->where(array('DATE_ID'=>$post['DATE_ID']))->find();
		if(empty($info)){
			$this->wrong("参数数据出错！");
		}
		for($i=0;$i<24;$i++){
		...
		$this->assign ('info', 				$info);
		$this->assign('time_list',		$time_list);	//时间列表
		$this->display('dfdate_add');
	}
	/*
	* 代付时间管理 删除
	**/
	public function dfdate_del() {
		$id = $_REQUEST['id'];
		if(empty($id)){
			$this->wrong('缺少参数');
		}
		$res = M('dfdate')->where(array('DATE_ID'=>$id))->delete();
		if(!$res){
			$this->wrong('删除失败');
		}
		$this->right('删除成功');
