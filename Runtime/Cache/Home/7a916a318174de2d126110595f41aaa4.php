<?php if (!defined('THINK_PATH')) exit();?><form id="pagerForm" method="post" action="">
	...
	<input type="hidden" name="post[submit]" value="baoxian_home"/>
...
<div class="pageHeader">
	<form method="post" action="<?php echo U('/Home/Daikuan/baoxian_home');?>" onsubmit="return navTabSearch(this);">
		<input type="hidden" name="post[submit]" value="baoxian_home">
		<div class="searchBar">
			<div class="header">
				<p>
				...
				<p class="maxcombox">
					<label>投保状态：</label>
					<select class="combox" name="post[KIND_STATUS]">
						<option value="">请选择</option>
						<?php if(is_array($kind_status)): $i = 0; $__LIST__ = $kind_status;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</p>
				<div class="clear"></div>
			</div>
			<div class="hbtn">
				...
				<a rel="Daikuan" target="navTab" href="/index.php/Home/Daikuan/baoxian_home" res_title="false"><button class="ch-btn-skin ch-btn-small ch-icon-refresh">重 置</button></a>
			</div>
		</div>
	</form>
</div>
<div class="pageContent">
	<div class="panelBar selbutton">
		<?php echo getaction_select(Daikuan,baoxian_home);?>
	</div>
	<table class="table" width="100%" layoutH="208">
		<thead>
		<tr>
			<th width="3%" align='center'>姓名</th>
			<th width="5%" align='center'>身份证</th>
			<th width="4%" align='center'>出生日期</th>
			<th width="2%" align="center">性别</th>
			<th width="4%" align="center">手机号码</th>
			<th width="3%" align="center">期望保障金额</th>
			...
			<th width="3%" align='center'>资产情况</th>
			<!-- <th width="5%" align='center'>人寿保险单</th> -->
			<!-- <th width="4%" align='center'>社保公积金</th> -->
			<th width="3%" align='center'>保单号</th>
			<th width="2%" align='center'>投保状态</th>
			<th width="6%" align='center'>投保险种</th>
			<th width="4%" align='center'>IP地址</th>
			<th width="6%" align='center'>创建时间</th>
		</tr>
		</thead>
		<tbody>
		<?php if(is_array($baoxian)): $k = 0; $__LIST__ = $baoxian;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr target="sid_target" rel="<?php echo ($vo["ID"]); ?>" class="<?php if($k%2 == 1): ?>bg<?php endif; ?>">
				...
				<td><?php echo ($money_type1[$vo[MONEY]]); ?></td>
				...
				<td><?php echo ($vo['POLICY_NUM']); ?></td>
				<td><?php echo ($kind_status[$vo['KIND_STATUS']]); ?></td>
				<td><?php echo ($vo['KIND_ACCIDENT']); ?></td>
	...
	<script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            tabscontent.find('a[note-type="getstatis"]').on('click',  function(){
                ...
                var url 	    = exportdata ? '/index.php/Home/Daikuan/baoxian_home' + exportdata + '&submit=baoxian_home&ajax=loading' : '/index.php/Home/Daikuan/baoxian_home?submit=baoxian_home&ajax=loading';
