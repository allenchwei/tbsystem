<?php if (!defined('THINK_PATH')) exit();?><div class="pageContent">
	<form action="/index.php/Home/Daikuan/luck_photo_add/navTabId/Daikuan" method="post" name="form" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone)">
		<input type="hidden" name="post[submit]" value="luck_photo_add">
		...
		<div class="pageFormContent" layoutH="<?php echo (C("tk_max_add_lay")); ?>">
			<div class="content30">
                ...
                <p style="width: 50%">
                    <label>跳转位置：</label>
                    <select class="combox" name="post[TYPE]">
                    	<option value="">请选择</option>
                        <?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if(($info['TYPE'] == $key)): ?>selected<?php endif; ?>><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </p>
				<p style="width: 100%">
					<label>跳转链接：</label>
					<input type="text" style="width: 70%" placeholder="背景图片不需添加链接地址" name="post[LINK_URL]" value="<?php echo ($info["LINK_URL"]); ?>"/>
				</p>
			</div>
			<div class="clear"></div>
			<div class="content50">
