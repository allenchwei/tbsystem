...
使用ThinkPHP，您可以更方便和快捷的开发和部署应用。当然不仅仅是企业级
...
[ 协议 ]
ThinkPHP遵循Apache2开源许可协议发布，意味着您可以免费使用ThinkPHP，
甚至允许把您的ThinkPHP应用采用商业闭源发布。
具体参考LICENSE.txt内容 
...
URL方式，支持不同的服务器和运行模式的部署，配合URL路由功能，让您随心
...
查询、统计查询、定位查询、多表查询、子查询、动态查询和原生查询，让您的
数据查询简洁高效。

动态模型：无需创建任何对应的模型类，轻松完成CURD操作，支持多种模型之间
的动态切换，让您领略数据操作的无比畅快和最佳体验。
...
模块分组：不用担心大项目的分工协调和部署问题，分组帮您解决跨项目的难题，
还可以支持对分组的二级域名部署支持。
...
开发和调试以及部署切换，让您轻松过渡，打造全新的开发体验。

RESTFul支持：REST模式提供了RESTFul支持，为您打造全新的URL设计和访问体验，
同时为接口应用提供了支持。
...
控制器扩展、Widget扩展在内的强大灵活的扩展机制，让您不再受限于核心的不足
