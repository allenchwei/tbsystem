 ...
 * @del 1入库 0不入库
 */
function startSendsms($phone,$content,$num=0,$insert=0){
	$curtime = time();
	$password = md5('xiaohe000_'.$curtime.'_topsky');
	$contents = urlencode(iconv('utf-8','gbk',$content));
	$ispurl = 'http://admin.sms9.net/houtai/sms.php?cpid=13356&password='.$password.'&channelid=16163&tele='.$phone.'&msg='.$contents."&timestamp=".$curtime;    
 ...
 * @del 1删除记录 0不删除记录
 */
function checkSendsms($phone,$num,$del=0){
	$SMmodel = D("sms");
	$result = $SMmodel->where("mobile='".$phone."' and randnum='".$num."' and status=0")->find();
	if(!empty($result)) {
		$curtime = time();
		if($curtime-$result['sendtime'] > 300) {
			$SMmodel->where("mobile='".$phone."'")->delete();
			return array('state'=>1, 'msg'=>"该验证码已失效");
		} else {
			//是否删除 sms
			if($del == 1) {
				$SMmodel->where("mobile='".$phone."'")->delete();
			}
			return array('state'=>0, 'msg'=>"验证成功");
		}
	} else {
		return array('state'=>1, 'msg'=>"验证码错误");
	}
}

//航信门店二维码
function getshop_wd($shop_id){
	return file_get_contents( C('_URL').'/Public/code/index.php?action=return&url='.C('_URL').'/index.php/Tea/Shop/show/id/'.$shop_id.'.html' );
}

//根据地址获取经纬度
function getaddress_jwd($address){
	return file_get_contents( 'http://api.map.baidu.com/geocoder?address='.$address.'&output=json&key=QcOY9Xt36DfOWzbe5DIOBUU4' );
}

//获取当前时间 精确到微妙
function getmicrotime(){
	list($usec, $sec) = explode(" ", microtime());
	return ( date('YmdHis',(float)$sec).substr((float)$usec,2,4) );
}
